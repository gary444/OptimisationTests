...
Vector2d function_1 (const double x, const double a, const double b) {
    return Vector2d(a*x*x+b*x, b*x*x+a*x);
}

Vector2d function_2 (const double x, const double c, const double d) {
    return Vector2d(c*x*x+d*x, d*x*x+c*x);
}
...
struct CustomSparseFunctor : public SparseFunctor<_Scalar, _Index>
{
	...
	int operator()(const InputType &x, ValueType& fvec)
	{
	    ...
	    for (Index m_i = 0; m_i < Index(this->values()); ++m_i) {
	        ...
	        const double y_val = y_data[m_i];
            // const Vector2d y_val = Vector2d( y_data[m_i*2], y_data[m_i*2+1] ) ;
	        ...
	        // if (m_i < Index(50)){
                ...
                auto res = function_1(x_val, a, b);
                // fvec[m_i*2+0] = y_val[0] - res[0];
                // fvec[m_i*2+1] = y_val[1] - res[1];
                fvec[m_i] = y_val - res[0];

                // std::cout << "result" << fvec[m_i*2+0] << ", " << fvec[m_i*2+1]  << std::endl;
        ...
        if (NUMERICAL_GRADIENT_CALCULATION){
            ...
            for (Index m_i = 0; m_i < Index(this->values()); ++m_i)
            {
                ...
                // if (m_i < 50) {
                    ...
                    pds(0,0) = x_val*x_val; // pd wrt a
                    pds(0,1) = x_val;       // pd wrt b
                    pds(1,0) = x_val;       // pd wrt a
                    pds(1,1) = x_val*x_val; // pd wrt b

                    // r = y_val - function_1(x_val, a, b);
...
//                 }
                ...
                // if (m_i < 50) {
                    ...
                    fjac.coeffRef(m_i, 0) = pds(0,0);
                    fjac.coeffRef(m_i, 1) = pds(0,1);
...
int main(int argc, char **argv) {
    ...
    // for LMA
    int n = 2;//num parameters
    int m = N;//num constraints (terms)
    Eigen::VectorXd x_vec (2); // parameter values (initial)    
    ...
    for (int i = 0; i < N; i++) {
        ...
        // if (i < N/2){
            ...
            auto true_result = function_1(x, ar, br); 
            y_data.push_back(true_result[0] + rng.gaussian(w_sigma));
        	// y_data.push_back(true_result[1] + rng.gaussian(w_sigma));

        // } else {
        //     auto true_result = function_2(x, cr, dr); 
        //     y_data.push_back(true_result[0] + rng.gaussian(w_sigma));
        // 	y_data.push_back(true_result[1] + rng.gaussian(w_sigma));
        // }
    }
    ...
    // {
    //     std::cout << x_data[i] << " : ";
    //     std::cout << y_data[i*2] << ", " << y_data[i*2+1] << std::endl;
    // }
    // return 0;
